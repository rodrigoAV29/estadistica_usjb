...
## 2. Ejemplos de analisis univariado y multivariado en una regresion logistica
...
tabla_reg_log_univ <- hipert_covid_1 |>
  tbl_uvregression(
    ...
    add_estimate_to_reference_rows = FALSE,
    pvalue_fun = ~style_pvalue(.x, digits = 3),
    stimate_fun = ~style_number (.x, digits = 2),
    label = list(
      edad ~"Edad(años)",
      sexo ~"Sexo",
      hipert ~"Hipertension",
      p_a_sistolica ~"Presion arterial sistolica",
      p_a_diastolica ~"Presion arterial diastolica (mmHg)",
...
**Paso 1: ajuste de modelo inicial**

```{r}
var_modelo = glm(
  desenlace ~ edad + sexo + hipert + p_a_sistolica + p_a_diastolica + 
    frec_cardiaca + frec_respirtatoria,
  data = hipert_covid_1,
  family = binomial(link = "logit")
 )
```

**Paso 2a : Realizamos modelo de seleccion variables**

```{r}
multi_forward <- var_modelo |>
  step(direction = "backrward", trace = FALSE)
```

**Paso 2b : Realizamos seleccion de variables**
...
**Paso 3c : Realizamos seleccion de variables**
...
### 2.7 Reporte de analisis univariado y multivariado

Tabla para resultados de regresion univariado (no ajustado)
...
Tabla de resultados de regresion multivariado (ajustado)
...
tabla_multi <- glm(
  ...
  tbl_regression(
    exponientate = TRUE,
    conf.int = TRUE,
    pevaluea_fun = ~style_pevalue(.x, digits =3),
    estimate_fun = ~style_number(.x, digits = 2)
    label = list(
      edad ~"Edad(años)",
      sexo ~"Sexo",
